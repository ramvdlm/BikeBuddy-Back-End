...
class DBFunctions {
  ...
  public function alert(){
	...
	public function recordIncident($latitude, $longitude, $ambulance, $biker_age, $bike_direction, $bike_position,
	  $biker_race, $biker_sex, $city, $county, $day, $location_of_crash, $time) {
		...
		$query = $db->query("INSERT INTO bike_crash(latitude, longitude, ambulance, biker_age, bike_direction,
			bike_position, biker_race, biker_sex, city, county, day, location_of_crash, time) VALUES('$latitude',
			'$longitude', '$ambulance', '$biker_age', '$bike_direction', '$bike_position', '$biker_race',
			'$biker_sex', '$city', '$county', '$day', '$location_of_crash', '$time')") or die(mysqli_error($db));
